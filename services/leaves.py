...
async def close_leave_subtree(
    ...
    # 🔧 AJUSTE CLAVE: cerrar sustitución en la baja padre
    if leave.parent_leave_id:
        parent = await session.get(Leave, leave.parent_leave_id)
        if parent:
            parent.substitute_teacher_id = None
            parent.substitute_end_date = end_date
