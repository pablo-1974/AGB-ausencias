        ...
        if sub:
            ...
            slots = await session.scalars(
                select(ScheduleSlot).where(
                    and_(ScheduleSlot.teacher_id == sid,
...
async def close_leave_cascade(
    ...
    # 1) Baja del profesor que se reincorpora
    lv = await _get_open_leave(session, teacher_id)
    if not lv:
    ...
    # 2) Obtener CADENA COMPLETA (incluye sustitutos)
    chain = [teacher_id] + await get_substitution_chain(session, teacher_id)

    # 3) Cerrar TODAS las bajas de la cadena
    for tid in chain:
    ...
    # 4) Romper TODAS las sustituciones entre niveles
    for i in range(len(chain) - 1):
        prof = chain[i]
        lv_prof = await _get_open_leave(session, prof)
        if lv_prof:
            lv_prof.substitute_teacher_id = None
            lv_prof.substitute_end_date = end_date
    ...
    # 6) Limpieza de “activos fantasma”
