...
async def clone_teacher_schedule(
    ...
    src_slots: List[ScheduleSlot] = (await session.execute(
        select(ScheduleSlot).where(ScheduleSlot.teacher_id == source_teacher_id)
    )).scalars().all()
    ...
    if replace_existing and pairs:
        for d, h in pairs:
            target_slots_same_pair: List[ScheduleSlot] = (await session.execute(
